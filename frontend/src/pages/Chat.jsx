...
const COLOR_MAP = {
  blue: {
    card: 'bg-blue-50 dark:bg-blue-500/10 hover:bg-blue-100 dark:hover:bg-blue-500/20 border-blue-200 dark:border-blue-500/30 text-blue-700 dark:text-blue-400',
    dot: 'bg-blue-500',
  },
  purple: {
    card: 'bg-purple-50 dark:bg-purple-500/10 hover:bg-purple-100 dark:hover:bg-purple-500/20 border-purple-200 dark:border-purple-500/30 text-purple-700 dark:text-purple-400',
    dot: 'bg-purple-500',
  },
  emerald: {
    card: 'bg-emerald-50 dark:bg-emerald-500/10 hover:bg-emerald-100 dark:hover:bg-emerald-500/20 border-emerald-200 dark:border-emerald-500/30 text-emerald-700 dark:text-emerald-400',
    dot: 'bg-emerald-500',
  },
};
async function fetchAIResponse(
  ...
  try {
    ...
    return {
      answer:
        "I'm sorry, I'm having trouble connecting to the legal analysis engine. Please try again later.",
      session_id: sessionId,
    };
  }
}
  ...
  useEffect(() => {
    const fetchDocDetails = async () => {
      ...
      try {
        ...
        // silent failure — UI shows generic messages
      }
    };
        ...
        <div className="flex-1 overflow-y-auto scrollbar-thin pb-32 pr-2 sm:pr-4">
          ...
          {isEmpty && (
            <div className="flex flex-col items-center pt-8 px-2">
              ...
              <h3 className="text-lg font-extrabold text-gray-900 dark:text-white mb-1 tracking-tight">
                How would you like your answer?
              </h3>
              <p className="text-sm text-gray-500 dark:text-gray-400 text-center mb-8 max-w-sm">
                Choose a response format below. The AI will reply and read it
                aloud using ElevenLabs voice.
              </p>
              ...
              <div className="w-full max-w-lg space-y-3">
                {FORMAT_OPTIONS.map((opt) => (
                  ...
                  >
                    <span
                      className={`w-3 h-3 rounded-full mt-1 flex-shrink-0 ${COLOR_MAP[opt.color].dot}`}
                    />
                    <div>
            ...
            <div className="space-y-8">
              {messages.map((msg) => (
                ...
                >
                  ...
                  {/* Bubble */}
                  <div
                    className={`flex flex-col max-w-[85%] sm:max-w-[75%] ${msg.sender === 'user' ? 'items-end' : 'items-start'}`}
                  >
                    {msg.sender === 'ai' && (
                      <div className="text-[10px] font-bold text-gray-500 tracking-wider uppercase mb-1 ml-1">
                        LegalDoc AI
                      </div>
                    )}
                    {msg.sender === 'user' && (
                      <div className="text-[10px] font-bold text-gray-500 tracking-wider uppercase mb-1 mr-1">
                        You
                      </div>
                    )}
        ...
        <div className="absolute bottom-0 left-0 right-0 pt-4 pb-2 bg-gradient-to-t from-gray-50 via-gray-50 dark:from-[#0F111A] dark:via-[#0F111A] to-transparent">
          ...
          <div className="text-center mt-3 px-4">
            <p className="text-[9px] sm:text-[10px] text-gray-500 font-medium">
              AI-generated legal advice should be reviewed by a professional. ©
              2024 LegalDoc Advisor.
            </p>
          </div>
        </div>
      </div>
