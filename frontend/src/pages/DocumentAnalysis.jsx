...
export default function DocumentAnalysis() {
  ...
  const analyzeDoc = useCallback(async () => {
    ...
    const BACKEND = (
      ...
      'https://livelegal-backend.up.railway.app'
    ).replace(/\/+$/, '');
    const userId =
      user?.id || sessionStorage.getItem('ll_user_id') || 'anonymous';
    try {
    ...
    fetch(`${BACKEND}/api/chat/analyze`, {
      ...
      .catch((err) => {
        ...
        if (err.name === 'AbortError') {
          // Aborted due to timeout or unmount
        } else {
          setError(err.message || 'Connection error. Please retry.');
        }
      })
    ...
    <AppLayout>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4 border-b border-gray-200 dark:border-[#1F2937] pb-4">
        <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 font-medium whitespace-nowrap overflow-x-auto w-full sm:w-auto pb-2 sm:pb-0">
          ...
          <span className="text-blue-600 dark:text-blue-500 truncate">
            {analysis ? analysis.document_type || 'Document' : 'Loading...'}
          </span>
        </div>
      ...
      {loading ? (
        <div className="py-20 text-center">
          <RefreshCw
            size={36}
            className="animate-spin text-[#9333ea] mx-auto mb-5"
          />
          <p className="text-[#a78bfa] font-medium">
            Analysing your legal document…
          </p>
        ...
        <div className="py-20 text-center px-4">
          ...
          <p className="text-red-400 font-medium mb-2 max-w-md mx-auto">
            {error}
          </p>
        ...
        <div ref={contentRef} className="px-1">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
            <div>
              <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-900 dark:text-white mb-2 tracking-tight break-all">
                {analysis.document_type || 'Unknown Document Type'}
              </h1>
              <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400">
                Analyzed just now{' '}
                <span className="mx-1 sm:mx-2 text-gray-300 dark:text-gray-600">
                  •
                </span>{' '}
                Legal Classification: {analysis.document_type || 'Contract'}
              </p>
            </div>
          </div>
            ...
            <div className="lg:col-span-4 flex flex-col gap-6">
                  ...
                  <div className="w-32 h-32 sm:w-40 sm:h-40 rounded-full border-8 sm:border-[12px] border-gray-100 dark:border-[#1F2937] relative flex items-center justify-center">
                    ...
                    >
                      <circle
                        ...
                        strokeDasharray="276"
                        strokeDashoffset={
                          276 - (276 * (analysis.severity_score || 0)) / 100
                        }
                        strokeLinecap="round"
                        ...
                        className={
                          analysis.risk_level === 'High'
                            ...
                            : analysis.risk_level === 'Moderate'
                              ? 'text-amber-500'
                              : 'text-blue-600 dark:text-[#1C36A4]'
                        }
                      />
                    </svg>
                    <div className="text-center">
                      <span className="text-3xl sm:text-4xl font-extrabold text-gray-900 dark:text-white">
                        ...
                        <span className="text-base sm:text-lg text-gray-400 dark:text-gray-500">
                          /100
                        </span>
                      </span>
                      <p
                        className={`text-[10px] font-bold tracking-wider uppercase mt-1 ${analysis.risk_level === 'High' ? 'text-red-600' : analysis.risk_level === 'Moderate' ? 'text-amber-500' : 'text-green-500'}`}
                      >
                  ...
                  {analysis.risk_factors?.slice(0, 3).map((factor, idx) => (
                    <div key={idx}>
                      <div className="flex justify-between items-end mb-1.5">
                        <span className="text-xs font-medium text-gray-500 dark:text-gray-400 truncate pr-2 max-w-[70%]">
                          {factor.label}
                        </span>
                        <span
                          className={`text-xs font-bold ${factor.points > 25 ? 'text-red-500' : factor.points > 10 ? 'text-amber-500' : 'text-blue-500'}`}
                        >
                          {factor.points} pts
                        </span>
                      </div>
                      <div className="w-full h-2 bg-gray-100 dark:bg-[#1A1D27] rounded-full overflow-hidden">
                        <div
                          className={`h-full ${factor.points > 25 ? 'bg-red-500' : factor.points > 10 ? 'bg-amber-500' : 'bg-blue-500'}`}
                          style={{
                            width: `${Math.min(factor.points * 2, 100)}%`,
                          }}
                        />
                      </div>
                    </div>
              ...
              <div className="bg-white dark:bg-[#151822] border border-gray-200 dark:border-[#1F2937] rounded-xl p-6 shadow-sm dark:shadow-soft flex-1 flex flex-col">
                <div className="flex items-center gap-2 mb-6">
                  ...
                  <h3 className="text-[15px] font-bold text-gray-900 dark:text-white tracking-wide">
                    AI Executive Summary
                  </h3>
                </div>
                ...
                <div className="bg-blue-50 dark:bg-[#1A1D27] border border-blue-100 dark:border-[#2A3143] rounded-lg p-4 flex gap-3 text-xs text-gray-700 dark:text-gray-400 leading-relaxed max-w-full">
                  <FileSearch
                    size={16}
                    className="text-blue-500 dark:text-gray-500 flex-shrink-0 mt-0.5"
                  />
                  <p className="min-w-0">
            ...
            <div className="lg:col-span-8 flex flex-col gap-6">
              ...
              <div className="bg-white dark:bg-[#151822] border border-gray-200 dark:border-[#1F2937] rounded-xl p-6 shadow-sm dark:shadow-soft flex-1">
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 gap-3">
                  <h3 className="text-xs sm:text-[15px] font-bold text-gray-500 dark:text-gray-400 uppercase sm:normal-case tracking-widest sm:tracking-wide">
                    AI Breakdown & Risk Factors
                  </h3>
                  <span
                    className={`text-[10px] uppercase font-bold tracking-wider px-3 py-1 rounded inline-flex items-center ${analysis.risk_level === 'High' ? 'bg-red-50 text-red-600 border border-red-200 dark:bg-red-500/10 dark:text-red-500 dark:border-red-500/20' : 'bg-amber-50 text-amber-600 border border-amber-200 dark:bg-amber-500/10 dark:text-amber-500 dark:border-amber-500/20'}`}
                  >
                  ...
                  {analysis.risk_factors && analysis.risk_factors.length > 0 ? (
                    analysis.risk_factors.map((factor, idx) => (
                      ...
                      >
                        ...
                        <div>
                          <h4 className="text-sm sm:text-[15px] font-bold text-gray-900 dark:text-white mb-1 tracking-wide">
                            {factor.label}
                          </h4>
                          <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 leading-relaxed capitalize">
                    ...
                    <p className="text-sm text-gray-500 italic">
                      No specific risk factors extracted.
                    </p>
                  )}
                </div>
              </div>
                ...
                <div className="bg-gray-50 dark:bg-[#0F111A] border border-gray-200 dark:border-[#1F2937] rounded-xl p-4 sm:p-5 flex-1 flex flex-col font-mono text-sm group relative">
                  <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-4 pb-3 border-b border-gray-200 dark:border-[#1F2937] gap-3">
                    <p className="text-xs text-gray-500 font-sans tracking-wide">
                      "Copy this template to your email response"
                    </p>
            ...
            <div className="lg:col-span-12 mt-2">
              ...
              <div className="w-full h-80 bg-gray-50 dark:bg-[#0F111A] border border-gray-200 dark:border-[#1F2937] rounded-xl rounded-tl-none relative overflow-hidden flex items-center justify-center group">
                ...
                <div className="absolute inset-4 border border-gray-300 dark:border-[#1F2937] border-dashed rounded bg-white dark:bg-[#151822] flex flex-col items-center justify-center text-gray-400 dark:text-gray-500">
                  ...
                  <p className="text-xs font-bold tracking-widest uppercase opacity-50 text-center px-4">
