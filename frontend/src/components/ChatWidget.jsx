...
import { MessageSquare, X, Send, Bot, User, Paperclip, ChevronDown, Volume2, VolumeX, Loader2, Sparkles, Mic, Type, Languages, Image as ImageIcon } from 'lucide-react';
...
const COLOR_MAP = {
  blue:    { btn: 'bg-blue-50 dark:bg-blue-500/10 hover:bg-blue-100 dark:hover:bg-blue-500/20 text-blue-700 dark:text-blue-400 border-blue-200 dark:border-blue-500/30', dot: 'bg-blue-500' },
  purple:  { btn: 'bg-purple-50 dark:bg-purple-500/10 hover:bg-purple-100 dark:hover:bg-purple-500/20 text-purple-700 dark:text-purple-400 border-purple-200 dark:border-purple-500/30', dot: 'bg-purple-500' },
  emerald: { btn: 'bg-emerald-50 dark:bg-emerald-500/10 hover:bg-emerald-100 dark:hover:bg-emerald-500/20 text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-500/30', dot: 'bg-emerald-500' },
};
async function fetchAIResponse(documentId, question, formatValue, languageCode, sessionId, userId) {
  try {
    ...
    return {
      answer: "I'm sorry, I'm having trouble connecting to the legal assistant. Please try again later.",
      session_id: sessionId
    };
  }
}
    ...
    <div className={`fixed right-4 sm:right-6 bottom-4 sm:bottom-6 w-[calc(100vw-32px)] sm:w-[390px] bg-white dark:bg-[#151822] border border-gray-200 dark:border-[#1F2937] rounded-2xl shadow-xl dark:shadow-[0_10px_40px_-10px_rgba(0,0,0,0.5)] z-50 flex flex-col transition-all duration-300 origin-bottom-right ${isMinimized ? 'h-[60px]' : 'h-[560px] max-h-[82vh]'}`}>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-bold text-gray-900 dark:text-white leading-none mb-0.5">LegalDoc AI</h3>
            <div className="flex items-center gap-1.5">
              <p className="text-[10px] text-gray-500 dark:text-gray-400 font-medium">Analyzing: {documentName}</p>
              {usingFallback && (
                <span className="text-[9px] font-bold bg-amber-100 dark:bg-amber-500/20 text-amber-600 dark:text-amber-400 px-1.5 py-0.5 rounded-full border border-amber-200 dark:border-amber-500/30 whitespace-nowrap">Browser Voice</span>
              )}
            </div>
          </div>
        </div>
          ...
          {isMuted && (
            <div className="mx-3 mt-2 px-3 py-1.5 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 rounded-lg flex items-center gap-2">
              ...
              <p className="text-[11px] text-amber-600 dark:text-amber-400 font-medium">
                Voice muted — speech is still generated. Tap <Volume2 size={10} className="inline mx-0.5" /> to hear it.
              </p>
            </div>
          )}
          ...
          <div className="flex-1 overflow-y-auto scrollbar-thin bg-white dark:bg-[#0F111A]">
            ...
            {isEmpty && (
              <div className="p-4 flex flex-col gap-3">
                ...
                {FORMAT_OPTIONS.map((opt) => (
                  ...
                  >
                    <span className={`w-2 h-2 rounded-full mt-1.5 flex-shrink-0 ${COLOR_MAP[opt.color].dot}`} />
                    <div>
            ...
            {!isEmpty && (
              <div className="p-4 space-y-4">
                {messages.map((msg) => (
                  <div key={msg.id} className={`flex gap-3 max-w-[92%] ${msg.sender === 'user' ? 'ml-auto flex-row-reverse' : ''}`}>
                    {/* Avatar */}
          ...
          <div className="p-3 bg-white dark:bg-[#151822] border-t border-gray-100 dark:border-[#1F2937] rounded-b-2xl">
            ...
            <div className={`relative flex items-end gap-2 bg-gray-50 dark:bg-[#0F111A] border ${isListening ? 'border-red-400 dark:border-red-500/50 ring-1 ring-red-400/30' : 'border-gray-200 dark:border-[#1F2937] focus-within:border-blue-500 dark:focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-500'} p-1.5 rounded-xl transition-all shadow-inner`}>
              ...
              <textarea
                ...
                placeholder={isListening ? (language === 'hi-IN' ? "सुन रहा हूँ..." : "Listening...") : (language === 'hi-IN' ? "हिंदी में पूछें..." : "Type or speak your question...")}
